...
class Environment:
    def __init__(self):
        ...
        self.expected_rental_requests = [0, 0, 20]
        self.expected_rental_returns = [10, 5, 0]
        self.capacity = [3, 2, 3]
        ...
        #self.P =  [[[(0,0,0) for i in range(self.capacity[0]+1)] for i in range(self.capacity[1]+1)] for i in range(self.capacity[2]+1)]
    ...
    def expected_return(self, state, action):
        ...
        for rental_rates in itertools.product(range(cars_available[0] + 1),
            ...
            for return_rates in itertools.product(range(current_capacity[0] + 1),
                ...
                returns += current_return_prob * (
                            current_rental_rewards + self.gamma * self.V[tuple(cars_available_eod)])
    ...
    def value_iteration(self, threshold=0.1):
        ...
        while True:
            ...
            for state_idx, state in enumerate(states):
                ...
                V[state] = temp
                self.policy[state] = ind
                delta = max(delta, abs(v - V[state]))
            t1 = time.time()
            iteration += 1
            print("Iteration: {}\tDelta: {}\tTE: {} sec".format(iteration, round(delta,2), round(t1 - t0,2)))
            if delta <= threshold:
                break
        print("==============Value Matrix====================")
        print(self.V)
        print("==========States and Optimal Action===========")
        for i,j in self.policy.items():
            print("State {} , Optimal Action: {}".format(i , j))
